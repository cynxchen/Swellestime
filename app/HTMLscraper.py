...
# Alice Li
# WHACKS Hack
# 10.03.2015 commented out funtions that my project does not need
# All credit and thanks to Samantha Voigt for being a cool mentor
...
class BrowserScraper():
    def __init__(self):
        self.soup = bs(urllib2.urlopen('https://webapps.wellesley.edu/course_browser/'))
    ...
    def saveDistributions(self, rowCells, courseDict):
        if rowCells[10].contents != []:
            if len(rowCells[10].contents) == 1: # if there is only one distribution
                courseDict['Distribution(s)'] = rowCells[10].string
            else:
                distList = []
    ...
    def saveDaysAndTimes(self, rowCells, courseDict):
        ...
        if (timeCell.contents != [] and dayCell.contents != []):
            ...
            if len(timeCell.contents) > 1: # if there is more than one meeting time
                ...
                timeList.append(timeCell.string)
...
#            self.saveDescriptionAndPrereqs(cells, indivClassDict)
            ...
            dictOfCourses[indivClassDict['CRN']] = indivClassDict
    ...
    def updateAll(self):
        ...
        self.writeCoursesFile(dictOfCourses)
        self.writeDistFile(self.getDistributions())
        self.writeSubjFile(self.getSubjects())
        self.writeDeptFile()
        return dictOfCourses
